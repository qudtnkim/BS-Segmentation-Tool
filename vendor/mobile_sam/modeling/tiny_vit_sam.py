...
# ---------------------------------------------------------------------------
# VENDORED: the upstream file imports DropPath / to_2tuple / trunc_normal_ from
# timm and register_model from timm.models.registry. timm is a heavy dependency
# whose install kept failing for users, and only these four symbols are ever
# needed. They are reimplemented below so this package has no dependency beyond
# torch. Behaviour is identical for inference.
# ---------------------------------------------------------------------------
...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    # torch.nn.init.trunc_normal_ exists since PyTorch 1.11 and matches timm's
    # implementation. Only reached during random init, never when loading a
    # checkpoint, so an approximate fallback is harmless on ancient torch.
    try:
...
class TimmDropPath(nn.Module):
    """Stochastic depth per sample. Inference-time no-op (training=False)."""

    def __init__(self, drop_prob=None, scale_by_keep: bool = True):
        super().__init__()
        self.drop_prob = 0. if drop_prob is None else float(drop_prob)
...
def register_model(fn):
    """timm's model-zoo registration decorator. Only used by the pretrained
    helper at the bottom of this file, which the SAM path never calls."""
    return fn
